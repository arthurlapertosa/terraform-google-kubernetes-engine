...
package confidential_gpu_public
...
func TestConfidentialGpuPublic(t *testing.T) {
	// projectID := testutils.GetTestProjectFromSetup(t, 1)
	bpt := tft.NewTFBlueprintTest(t,
		tft.WithVars(map[string]interface{}{"project_id": "project-1-id-43298943204"}),
	...
	bpt.DefineVerify(func(assert *assert.Assertions) {
		...
		keyName := strings.Split(bpt.GetStringOutput("kms_key_name"), "/")
		keyNameStr := strings.Join(keyName[len(keyName)-4:], "/")
		...
		g := golden.NewOrUpdate(t, op.String(),
			golden.WithSanitizer(golden.StringSanitizer(keyNameStr, "KEY_NAME")),
		...
		validateJSONPaths := []string{
			...
			"nodeConfig.accelerators.acceleratorType",
			...
			"nodePools.config.accelerators",
			"nodePools.config.accelerators.acceleratorType",
			"nodePools.config.accelerators.acceleratorCount",
